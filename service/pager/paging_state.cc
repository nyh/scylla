...
service::pager::paging_state::paging_state(partition_key pk, std::experimental::optional<clustering_key> ck,
        uint32_t rem, utils::UUID reader_recall_uuid)
        : _partition_key(std::move(pk)), _clustering_key(std::move(ck)), _remaining(rem), _reader_recall_uuid(reader_recall_uuid) {
}
