...
static atomic_cell make_empty(const atomic_cell_view& ac) {
    // TODO: can we clone ac's liveness without all these cases?
...
// issue #3362. The following function takes a full value c (taken from the
// base table) for the given column col, and if that column is a virtual
// column it modifies c to remove the unwanted value.
static void maybe_make_virtual(atomic_cell_or_collection& c, const column_definition* col) {
    ...
    if (col->type->is_atomic()) {
        ...
        auto ctype = static_cast<const collection_type_impl*>(col->type.get());
        if (ctype->is_list()) {
