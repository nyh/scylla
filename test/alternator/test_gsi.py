...
from util import create_test_table, random_string, random_bytes, full_scan, full_query, multiset, list_tables, new_test_table
...
# the key's type, some is definitely type-specific - namely, the "computed
# function" which deserializes the value from the base to put it in the view
# row. So test_table_gsi_6 is a table which has six GSIs, each one with
# one of the three legal AttributeType (S, B, and N) for its partition key
# or clustering key.
@pytest.fixture(scope="module")
...
# (as well as empty value, so no write, for other types). Also check failure
# cases (wrong types, and empty string).
# After them we have a test (test_gsi_6) for reading from these GSIs, not
# just writing.
# As explained in a comment above # test_table_gsi_6, the main goal of these
...
def test_gsi_6_write_s(test_table_gsi_6):
    p = random_string()
    s = random_string()
    test_table_gsi_6.put_item(Item={'p': p, 's': s})
...
# happens to store key attributes for LSIs and GSIs differently, when we
# have both, both are usable. This is a delicate corner case, which should
# be tested so it doesn't regress.
#
# In particular, at the time of this writing, when "x" is an LSI key, "x"
# becomes a full-fleged column in the schema. Yet, when there is also a
# GSI, supposedly the GSI uses a "computed column" to extract x from the
# map ":attrs" of un-schemad attributes. So how does this even work?
...
# "cleaned up" in the future, this test is important.
#
