...
future<json::json_return_type> rmw_operation::execute(service::storage_proxy& proxy,
        ...
        bool needs_read_before_write,
        stats& stats) {
    // FIXME: the following 3 options keep different mechanisms for doing
    // read-before-write in the code, and different ways of combining write-
    // only and read-before-write concurrently. Eventually we should choose
    // among these options, and/or make it configurable globally, per-table
    // or per-attribute. If we decide to completely remove one of the options
    // some of the code paths below can also be removed. The option
    // unsafe_read_before_write is a prime candidate for removal.
    bool forbid_rmw = false; // Whether to always forbid read before write
    bool non_rmw_uses_lwt = true; // Whether write-only updates also use LWT (if false - a normal write).
    bool unsafe_read_before_write = false; // Unsafe mode doing separate reads and writes.
    ...
    if (needs_read_before_write) {
        ...
        if (unsafe_read_before_write) {
            // This is the old, unsafe, read before write which does first
            // a read, then a write. FIXME: remove this mode entirely.
    ...
    } else if (!non_rmw_uses_lwt || unsafe_read_before_write) {
        // If there is no read before this write and non_rmw_uses_lwt is off,
        // writes will be a normal write without LWT. We also do this, of
        // course, when unsafe_read_before_write is on.
    ...
    // NYH TODO: should we bounce the request to a different shard, as in the patch "lwt: Process lwt request on a owning shard"?
